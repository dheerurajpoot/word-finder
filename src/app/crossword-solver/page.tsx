...
export default function CrosswordSolverPage() {
  ...
  const handleSolve = async () => {
    ...
    try {
      ...
      const patternLower = pattern.toLowerCase().replace(/[^a-z?_-]/g, "")
      const searchParams: any = {}

      // Handle different pattern formats
      if (patternLower.includes("?") || patternLower.includes("_") || patternLower.includes("-")) {
        // Pattern with wildcards
        const letters = patternLower.replace(/[?_-]/g, "").split("").join("")
      ...
      const filteredResults = searchResults.filter((word) => {
        if (pattern.length > 0 && word.length !== pattern.length) return false
        ...
        for (let i = 0; i < pattern.length; i++) {
          ...
          if (patternChar !== "?" && patternChar !== "_" && patternChar !== "-" && patternChar !== wordChar) {
            return false
          }
        }
        return true
  ...
  const examplePatterns = [
    { pattern: "C_T", description: "3-letter word starting with C, ending with T" },
    { pattern: "_O_E", description: "4-letter word with O as 2nd letter, E as last" },
    { pattern: "?R??D", description: "5-letter word with R as 2nd letter, D as last" },
      ...
      <div className="container mx-auto px-4">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="text-center mb-12">
            ...
            <h1 className="text-4xl font-bold text-gray-800 mb-4">Crossword Solver</h1>
            <p className="text-xl text-gray-600 mb-2">Find words that fit your crossword puzzle patterns</p>
            <p className="text-gray-500">Use ? _ or - for unknown letters</p>
          </div>
            ...
            <CardContent className="p-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                  ...
                  />
                  <p className="text-xs text-gray-500 mt-1">Use _, ?, or - for unknown letters</p>
                </div>
                <div>
              ...
              <div className="flex gap-4">
                ...
                <Button onClick={clearAll} variant="outline" className="h-12 px-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                  {results.map((word, index) => (
                    ...
                    >
                      ...
                      <div className="text-sm text-gray-500">{word.length} letters</div>
                      <div className="mt-2 text-xs text-gray-400">
                        Pattern:{" "}
                        {pattern.split("").map((char, i) => (
                          <span
                            key={i}
                            className={char === "?" || char === "_" || char === "-" ? "text-orange-500" : ""}
                          >
                            {word.word[i] || char}
                          </span>
                        ))}
                ...
                <div className="space-y-4">
                  {examplePatterns.map((example, index) => (
                    <div key={index} className="border-l-4 border-orange-200 pl-4">
                      <div className="font-mono font-bold text-orange-600">{example.pattern}</div>
              ...
              <CardContent>
                <div className="space-y-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <div className="font-semibold text-sm">Use Known Letters</div>
                      <div className="text-xs text-gray-600">Enter letters you know, use ? for unknowns</div>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <div className="w-6 h-6 bg-orange-100 rounded-full flex items-center justify-center text-orange-600 font-bold text-xs mt-0.5">
                    ...
                    <div>
                      <div className="font-semibold text-sm">Consider Word Length</div>
                      <div className="text-xs text-gray-600">Count squares carefully for accurate patterns</div>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <div className="w-6 h-6 bg-orange-100 rounded-full flex items-center justify-center text-orange-600 font-bold text-xs mt-0.5">
                    ...
                    <div>
                      <div className="font-semibold text-sm">Use Crossing Words</div>
                      <div className="text-xs text-gray-600">Letters from intersecting words help narrow options</div>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <div className="w-6 h-6 bg-orange-100 rounded-full flex items-center justify-center text-orange-600 font-bold text-xs mt-0.5">
                    ...
                    <div>
                      <div className="font-semibold text-sm">Think About Clues</div>
                      <div className="text-xs text-gray-600">Consider wordplay, synonyms, and themes</div>
                    </div>
                  </div>
                </div>
              </CardContent>
