...
function SearchContent() {
  ...
  useEffect(() => {
    ...
    // Perform initial search if letters are provided
    if (searchParams.get("letters")) {
      handleSearch()
    }
  }, [searchParams])

  const handleSearch = async () => {
    if (!letters.trim() && !starts.trim() && !ends.trim() && !contains.trim()) {
      return
    }
    ...
    try {
      ...
      const results = await searchWords(searchParams)
  ...
  const generateMockWords = (
    ...
    return baseWords
      .filter((word) => {
        if (starts && !word.startsWith(starts.toLowerCase())) return false
        if (ends && !word.endsWith(ends.toLowerCase())) return false
        if (contains && !word.includes(contains.toLowerCase())) return false
        if (length && word.length !== Number.parseInt(length)) return false
        return true
      })
      .map((word) => ({
        word,
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b">
        <div className="container mx-auto px-4 py-3">
          <nav className="text-sm text-gray-600">WordTips / Unscramble / Results</nav>
        </div>
      </div>
          ...
          <div className="lg:col-span-3">
            {words.length === 0 && !loading ? (
              <div className="text-center py-12">
                ...
                <p className="text-gray-600 mb-8">
                  Sorry, there are no exact matches, but adjusting the in your search can help you get more results. The
                  following words add extra letters that you might find on your game board.
                </p>
                <p className="font-semibold mb-8">Showing words adding 3 wildcards</p>
              </div>
            ) : null}
            ...
            {loading ? (
              <div className="text-center py-12">
                ...
                <p className="mt-4 text-gray-600">Searching for words...</p>
              </div>
            ) : null}

            {Object.entries(groupedWords).map(([lengthKey, wordsInGroup]) => (
              <div key={lengthKey} className="mb-8">
                {groupByLength && lengthKey !== "all" && (
                  <div className="bg-green-400 text-white px-4 py-2 rounded-t-lg">
                    <h3 className="font-semibold">{lengthKey} Letter Words</h3>
                  </div>
                )}

                <Card className={groupByLength && lengthKey !== "all" ? "rounded-t-none" : ""}>
                  <CardContent className="p-6">
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3">
                      {(showMore ? wordsInGroup : wordsInGroup.slice(0, 20)).map((word, index) => (
                        <Badge
                          key={index}
                          variant="secondary"
                        ...
                        >
                          {word.word}
                          <sub className="ml-1 text-xs">{word.score}</sub>
                        </Badge>
                      ))}
                    </div>

                    {wordsInGroup.length > 20 && !showMore && (
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <Card>
              ...
              <CardContent>
                ...
                <div className="mt-4">
                  <div className="flex items-center space-x-2">
                    <Checkbox id="group-length" checked={groupByLength} onCheckedChange={(checked) => setGroupByLength(checked === true)} />
                    <Label htmlFor="group-length">Group by Length</Label>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <RadioGroup value={dictionary} onValueChange={setDictionary}>
                  <div className="flex items-center space-x-2">
                    <RadioGroupItem value="words-with-friends" id="wwf" />
                    <Label htmlFor="wwf">Words With Friends</Label>
                  </div>
                  <div className="flex items-center space-x-2">
                    <RadioGroupItem value="scrabble-us" id="scrabble-us" />
                    <Label htmlFor="scrabble-us">Scrabble US</Label>
                  </div>
                  <div className="flex items-center space-x-2">
                    <RadioGroupItem value="scrabble-uk" id="scrabble-uk" />
                    <Label htmlFor="scrabble-uk">Scrabble UK</Label>
                  </div>
                  <div className="flex items-center space-x-2">
                    <RadioGroupItem value="all" id="all" />
                    <Label htmlFor="all">All Dictionaries</Label>
                  </div>
                </RadioGroup>
              </CardContent>
            </Card>
